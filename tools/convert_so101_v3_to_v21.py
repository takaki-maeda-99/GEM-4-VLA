"""Convert takaki99/test_so101 (LeRobot v3.0) to v2.1 layout consumable by
the lerobot==0.3.3 installed in this venv, and drop failed/deleted episodes.

Why: lerobot 0.3.3 expects ``meta/episodes.jsonl`` + ``meta/tasks.jsonl``;
takaki99/test_so101 ships only the v3.0 parquet equivalents and is not
tagged with any version branch. We also need to filter out
``success=False`` and ``deleted=True`` episodes (62/73 keep) and renumber
the remaining episodes contiguously 0..N-1 — non-contiguous
``episode_index`` values would index out of bounds against the
positionally-indexed ``episode_data_index`` LongTensor built by lerobot
(see comment in LeRobotLiberoDataset about the same gotcha).
...
def main():
    ...
    p.add_argument("--repo_id", default="takaki99/test_so101")
